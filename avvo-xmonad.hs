...
main = do
  ...
  xmonad $ docks defaultConfig
    {
    manageHook = manageDocks <+> insertPosition End Newer <+> manageHook defaultConfig
    , layoutHook = avoidStruts $ spacingRaw True (Border 10 10 10 10) True (Border 10 10 10 10) True $
    ...
    , (( mod4Mask .|. shiftMask , xK_y), spawn "/home/stefano/.xmonad/layout_switch_it.sh")
    -- Mod-u allows windows to cover the system bar
    , (( mod4Mask , xK_u), sendMessage ToggleStruts)
    , (( mod4Mask , xK_p), spawn "dmenu_run -fn xft:Inconsolata:size=14:antialias=true")
